  ...
  return(
      <>
        <tr key={reservation_id}>
          ...
          <td >{status === "booked" ? (<Link to={`/reservations/${reservation_id}/seat`}>
            <button href={`/reservations/${reservation_id}/seat`} type="button" className="btn btn-sm table-condensed btn-secondary m-1">
              Seat
            </button>
          </Link>):""}</td>
          <td>{status === "booked" ? (<Link to={`/reservations/${reservation_id}/edit`}>
            <button href={`/reservations/${reservation_id}/edit`} type="button" className="btn btn-sm table-condensed btn-secondary m-1">
              Edit
            </button>
          </Link>):""}</td>
          <td> {status === "booked" ? (<button data-reservation-id-cancel={reservation_id} className="btn btn-sm table-condensed btn-secondary m-1" onClick={() => handleCancel(reservation_id)}>
            Cancel
            </button>):""}</td>
        </tr>  
      </>
